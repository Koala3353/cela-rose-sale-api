...
exports.queueOrderWrite = queueOrderWrite;
...
const orderWriteQueue = [];
let orderWriteActive = false;
async function processOrderQueue() {
    if (orderWriteActive || orderWriteQueue.length === 0)
        return;
...
function queueOrderWrite(sheetId, sheetName, rows) {
...
async function fetchSheetData(sheetId, sheetName, apiKey) {
    try {
        ...
        if (auth) {
            ...
            sheetsClient = googleapis_1.google.sheets({ version: 'v4', auth: client });
        }
        else {
            sheetsClient = googleapis_1.google.sheets({ version: 'v4' });
        }
        const params = {
 ...
 */
function parseProductsData(values) {
...
async function appendToSheet(sheetId, sheetName, rows, useQueue = false) {
 ...
 */
async function fetchUserOrdersFromSheet(sheetId, sheetName, userEmail, apiKey) {
    try {
        ...
        for (let i = 1; i < rawData.length; i++) {
            ...
            if (orderEmail !== normalizedUserEmail)
                continue;
            const order = {
 ...
 */
async function updateStockCounts(sheetId, sheetName, stockUpdates // Map of productId -> newStock
) {
    const auth = getAuth();
    if (!auth) {
    ...
    try {
        const client = await auth.getClient();
        const sheetsClient = googleapis_1.google.sheets({ version: 'v4', auth: client });
